...
def Stage0(config): 
    ...
    imageh=config['image_height']
    imagel=config['image_length']
    image_type=config['image_type']
    
    if type_dataset=='folder': #Incase dataset is a set of images in a folder
        final_image_list = []
        for img in glob.glob(config['genral_path_incase_of_folder']+'/*.'+image_type):
            temp_image= cv2.imread(img,0)
            temp_image = cv2.resize(temp_image, (imagel, imageh)) 
